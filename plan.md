...
5. 同时Edit多个file,审批的时候能不能直接全部展示diff修改,然后用户选择,选择项就是要多一个选项是用户选择逐个审批才进入逐个审批的方式,逐个审批的话就把之前edit的diff展示全部删掉,换成现在的单个慢慢加.(batch edit是不是也可以直接一起发送给auto模型来判断,省交互次数,你看下当前batch edit是要逐个给auto审批吗)
6. 比如需要审批的命令太长,这个窗口不能滚动,看不到下面的权限弹窗了,你看下最大限制显示多少行,最好支持滚轮查看,或者用类似diff的规则?长命令审批前先渲染再看是否同意来重新渲染.
