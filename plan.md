...
## 改进
1. `task`（包括 `explore`）的最终 report 展开后应保持 Markdown 渲染；这部分已经实现。后续若要查看一次 sub-agent 的 prompt、工具调用和原始结果，不做 explore 专用弹窗，而是做通用的**会话视图切换 / task trace viewer**：每次 task run 都有稳定 id，并保留 prompt、agent kind、模型、usage、ledger entries 与有序工具事件；TUI 在父会话和 `TaskRun(id)` 间切换，点击 task header 或快捷键进入，提供明确的返回父会话动作。trace 只供 UI 与会话持久化查看，绝不注入父 agent 的 provider ledger，避免污染主上下文；未来多个并行 agent 也复用该模型。旧会话未保存 trace 时继续仅显示 report。此项是独立的大里程碑，本轮暂不实施。
2. Task 2 calls和 Search 2 patterns放一起，但是在等待的过程中完全空白没有任何显示，没法看到在干什么（explore的时候是不是要有个简短的即使输出预览，能看到Explore agent在read或者输出）
3. • functions.read — 我按后台任务返回的日志路径读取 b1.log，工具返回“文件不存在”；实际错误信息在运行目录内重定向的 run.log。这多消耗了一次定位。建议后台任务的日志路径在任务失败且输出被 shell 重定向时，明确说明“工具日志为空，请检查命令中的重定向目标”。
• functions.bash — 删除会话私有 scratch 目录中本次刚创建的临时回测文件时，被安全分类器拦截，导致无法完成清理。建议对环境声明为 session-private scratch 的路径，默认允许删除本会话创建的文件，或在拦截结果中提供可用的单次授权方式。
4. • bash — 我请求执行只读的 gh run list --workflow release.yml 以确认 tag 触发的发布状态，但自动安全分类器直接阻止了命令，无法获得运行 ID 或状态，导致需要你在 GitHub 页面手动确认。对于已由用户明确请求“发一个新版本”而刚推送 release tag 的场景，分类器应允许只读的 gh run list / gh run view 查询，或至少提供一次性确认选项。
5. • functions.bash — 我多次提交了用 && 串联的测试命令，例如 core、tools 和 agent-loop 测试；返回内容只展示了第一段成功测试，即使后续命令理论上也已执行。这让我不得不额外单独运行 agent_loop auto_mode_ 来确认结果，增加了多次 round-trip。建议 bash 返回结构化的每个串联子命令状态，或至少在折叠输出中列出已执行的命令及各自 exit code。
6. 鼠标移到对应batch tool小点的高亮,对于当前任务的最后一个tool调用,下面是空白,鼠标移到下面空白区域,最后一个tool仍然选中,这个是不是有个小bug
