...
## 改进
1. • bash (gh run watch) — 我等待 Release 工作流时，命令持续输出每 3 秒一轮、内容高度重复的完整 job 状态，累计 6,713 行并被截断保存到 scratch 输出文件；虽然末尾保留了成功结果，但这造成了大量无用上下文。建议 harness 对持续性命令的重复输出做增量折叠，或为 bash 提供“仅最终状态/仅状态变化”的输出模式。
2. markdown链接得支持ctrl+鼠标点击可以跳转到浏览器打开.
3. /agents面板也支持鼠标点击换模型
4. @选择文件的时候,按enter应该和按tab一个功能,而不是直接发送了.

### 已确认的改进实施计划

#### 第三阶段：安全的末尾追加工具（✅ 已完成）

7. **新增 `append`，而不放宽覆盖型 `write`（#8）** — 已落地，实施中确定的三个配套决策：
   - `append(path, content)` 仅支持 UTF-8 文本、原样追加不自动补换行；已有文件要求 freshness 已见当前版本 hash（**部分读过即可**，外部改动拒绝）；**缺失文件直接创建**（含建父目录，输出注明，全模型自著故记为完整可见）。中间插入仍归 `edit`。
   - **`write` 覆盖门同步收紧为「完整看过当前版本」**（freshness 新增 `Visibility` 四态查询）：partial 视图得到列出已见行段的自愈错误——否则"部分读 → append → 全量 write"仍可摧毁没见过的内容，「不能让后续全量 write 绕过保护」才真正成立。
   - **`edit` 成功后按 `record_read` 记录已回显的 snippet**（新 hash + snippet 行段，不是 `record_write` 的假 full）：没有这条，read → edit → append 会被 append 的门误判成外部改动。同理 `append` 成功后经 `record_append` 记录"回显范围 + 携带旧可见性"，partial 永不静默升格 full（并入后恰好覆盖全文件时才升格）。agent loop 的同批次冲突注释改挂在 match-miss 标记上（成功 edit 记录新 hash 后，"have not read" 提示在 lane 内不再出现）。
