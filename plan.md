...
7. 调用Sub agent的时候转录记录在任务概要的后面也展示sub-agent使用的模型，注意美观度。
8. gpt订阅有图片生成模型吗
9. 模式切换是不是改成切换多少秒不再切换后（一般要切应该会shift_tab一直切）就自动生效更靠谱，不一定非要强行等到下一次提交后(模式切换加的note可不可以仍然还是延迟到下一次提交一起发，只是模式切换带来的权限改动可以直接判定)
