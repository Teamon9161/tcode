...
5. 用户在对话框输入note内容的时候，移动光标对导致光标右边的字往后面移动，光标自身不应该不占位置吗，现在感觉还占了半个空格的感觉
6. 比如说像默认定义的agent explore.md，如果允许提权 ask user, 最后是不是也要遵守最外面的mod规则，比如plan, accept-edits，default都可以问，但是auto这个可以问吗你觉得，我们现在auto是不是尽量让模型先问好，然后后面允许或拒绝就交给auto 分类呢。
