...
## 改进
1. • bash — 我将“删除本次失败测试留下的已知 /tmp 临时目录”与后续格式化/测试合并执行，安全分类器因递归删除拦截了整条命令，导致验证必须额外再跑一轮。更好的行为是允许对 agent 在本次会话创建且路径精确匹配的临时目录自动清理，或在拒绝时继续执行同一命令中可证明无害的后续子命令。
2. • edit — 我尝试并行补充测试时，一个精确替换因实际代码上下文不同而失败；工具返回了最近匹配片段，但仍需要额外一次读取和修正，且另一次替换在相同函数名附近落到了生产代码区域。可以让 edit 在近似匹配提示中同时返回更大的唯一上下文，或提供“限定在测试模块内”的结构化范围参数，减少同名锚点导致的误插入。
3. 看codex项目如何用reset次数,
4. The Monitor tool lets Claude watch something in the background and react when it changes, without pausing the conversation. Ask Claude to:
...
For most watches, Claude writes a small script, runs it in the background, and receives each output line as it arrives. For a server that already pushes events, Claude can open a WebSocket instead of running a script.
...
When Monitor runs a command, it uses the same permission rules as Bash, so allow and deny patterns you have set for Bash apply here too. The WebSocket source has its own approval prompt.
5. web_fetch支持指定模型来总结以及相应prompt,而不是给原始返回.
