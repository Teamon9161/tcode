...
2. The Monitor tool lets Claude watch something in the background and react when it changes, without pausing the conversation. Ask Claude to:
...
3. web_fetch支持指定模型来总结以及相应prompt,而不是给原始返回.
4. grep — 我以 path: crates/tcode-tui/src、glob: app.rs 搜索鼠标处理代码时返回“0 files scanned”，但该目录中确实存在 app.rs；改为直接传入完整文件路径才得到结果。这多花了一次搜索。建议让 glob 在已限定目录下支持裸文件名，或在无匹配时提示 glob 是相对于哪个目录匹配。
5. 单个Explore的时候
Explore · 梳理 TUI hover 渲染入口与遗漏区域  (4 lines)
      └ Read 2 ranges · Search 2 patterns
最开始这个Explore没有像其他Tool名称那样绿色显示
6. Environment info
Auto-loaded
280 tokens
