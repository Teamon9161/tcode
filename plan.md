...
- `[agents.<kind>]`（`explore`/`plan`/`general`/`auto`/`suggest`/`vision`/`fetch`）：给 sub-agent 与辅助角色钉模型，`profile`/`model`/`effort` 三个可选字段，未写的继承父模型选择。`fetch` 是唯一"未钉即关"的角色（web_fetch 返回原文而非回退主模型）。见下 M8。
