...
## 改进
1. `task`（包括 `explore`）的最终 report 展开后保持 Markdown 渲染；查看 sub-agent 不做专用弹窗，而是做通用的 **task trace viewer**。每次 run 有稳定 id，并持久化 parent call、父 agent 给出的单行任务概要、完整 prompt、agent kind、模型、usage、ledger entries 与执行期批次标签。trace 只供 UI/会话持久化查看，绝不注入父 agent 的 provider ledger；未来多个并行 agent 复用同一模型。父会话的 task 调用卡片在 task 启动时立即附上可折叠的概要；运行期间其下方有一条始终可见、随事件替换的当前活动行，展开后显示最近工具步骤；task 结束时活动行删除并原位替换为最终 Markdown report（去除仅供 harness 使用的统计前缀）。卡片单击展开/折叠，450ms 内第二次点击才进入 trace。trace 顶部直接按 user prompt 样式展示完整 task prompt，并跳过 ledger 中那条重复的初始输入；后续子 agent 消息仍完整 replay。旧 trace 缺少 summary 时从 prompt 生成兼容回退。
2. 工具批次等待时不再空白：`update_progress` 与 task 组成一个**常驻 agent tree**，在主会话和任一 task trace 中都显示。tree 只在有运行中的 sub-agent 时出现；最后一个结束后隐藏，已结束 task 继续通过父 task 卡片查看。root `main` 单击立即返回主会话；task child 与 main 同级不缩进，显示 agent 类型、父模型按 prompt 同语言给出的极简概要（可自然换行）、运行时间与 token 数；当前 read/search/写作/思考活动只在父 task 卡片的可见状态行显示。单击 child 直接切换到该 task trace，不在 tree 展开步骤或报告。父 task 卡片与 tree 行都使用既有 hover 背景/高亮。`Esc` 从 trace 回主会话。`/views` 明确保留给未来的并行顶层 session 切换，绝不列出该 session 的 sub-agent；当前只有一个 session 时显示无其他活动 session 的提示。
3. 改成run之后，现在权限弹窗经常是allow Run(Powershell)，是不是哪里没处理好。
4. 已合并到 #1 并实现：task trace 的父 agent prompt 直接按 user prompt 显示，不再默认折叠，也不会在 replay 时重复一份。
5. view_image的时候，不要把输出结果也直接展示在这一行中。
6. plan模式 tab 加note的时候看不出我是选了哪个选项，note的时候选项要保持选中吧
7. 如果之前不是plan mode，或者resume的对话，然后现在是plan mode，这种情况是不是得插入告诉模型，要用plan的工具，要不然感觉模型还是发文本。
8.  functions.edit（并行编辑会话记忆） — 我同时提交了项目内 plan.md 和项目外 Auto Memory 的编辑；整批被 Auto Mode 拒绝，因为 memory 文件位于 trusted scratch root 之外，导致本应独立安全的 plan.md 编辑也没有执行，额外增加了一次编辑调用。更好的行为是让独立批次逐项执行已允许的项目内编辑，并只拒绝外部 memory 那一项。

### 已确认的改进实施计划

#### 第三阶段：安全的末尾追加工具

7. **新增 `append`，而不放宽覆盖型 `write`（#8）**
   - 新工具 `append(path, content)` 仅支持 UTF-8 文本并把给定字节原样追加到文件尾；不自动补换行。
   - 对既有文件，在追加前读盘并要求 freshness tracker 已见同一 hash（允许部分读取，但外部改动后拒绝）；追加成功后记录新 hash 为非完整可见，不能让后续全量 `write` 绕过保护。
   - 返回实际追加内容和文件末尾带行号 preview；正常声明 `touches()`，复用 checkpoint、rewind、权限与同文件批处理 lane。已有文件中的中间插入仍应使用精确匹配 `edit`，未来若需要另设计 anchor 插入，不把它混进 `write`。
   - 触及：`crates/tcode-tools/src/fs_tools.rs`、`lib.rs`、`crates/tcode-tui/src/render.rs`、必要时 `freshness.rs`/`agent/mod.rs`；覆盖 partial-read → edit → append、外部改动拒绝、freshness 不误标、checkpoint rewind 与同文件批次顺序。
