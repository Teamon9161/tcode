...
| 工具 | 要点 |
|---|---|
| `read` | offset/limit + 行号；经 Freshness Tracker 去重；识别图片按 magic bytes 返回 image block（去重、>5MB 拒绝）；大输出/后台日志落 scratch 文件用 read 分页 |
...
| `update_plan` / `ask_user` / `add_note` | 交互工具；`ask_user` 支持多问题分页 |
...
- **输入框粘贴即发送（待实测）**：应用层已把 Ctrl/Alt+V 统一走 `paste_from_clipboard`、Ctrl+C 只做中断阶梯。若仍"粘贴即发送"，属终端把 bracketed paste 的换行当 Enter，只能终端层解决。
...
## M8：sub-agent 换模型、思考可展开、连接诊断、工具提速、隐藏命令

五项改进的落地结论（细节见代码，此处只留设计判断）：

1. **sub-agent 可跑另一个模型**（`/agents` 或 `[agents.<kind>]`）。侦察是"大量读 + 归纳"，本就适合便宜快模型，而且它的上下文根本不进主窗口——省下的是钱不是缓存。未配置的 kind 继续共享父 `ModelCell`（跟随 `/model` 热切换）；配置了就**钉死**，`/model` 不动它——"explore 永远走 flash"正是要这个语义。字段级继承父 selection：只写 `model` 时**去找真正提供该模型的 profile**（与 `--model` 不带 `--profile` 同规则，否则会把 deepseek 的模型 id 发给 codex 端点，用户要到第一次 sub-agent 调用才发现）；换了模型则不继承父 effort（那是另一个模型的刻度，属于猜）。配置写错不阻塞启动：warn 后该 kind 退回父模型。子代理报告行带上实际模型名。
   - **`/agents` 复用 `/model` 的选择器**：第一步选 agent（列出各自现在跑什么），第二步就是同一张 模型 × effort 网格，只是多一行 "inherit — follow the main model"（选它即解除钉住）。Esc 从第二步退回第一步，不是直接关掉。pin 写进 **state.toml**（程序只写这个文件），overlay 在手写的 `[agents.*]` 之上；空表项 = 显式 inherit，这样 UI 才可能解除 config.toml 里的钉住。运行时共享 `AgentModels`（与 `ModelCell` 同构的可换手柄），所以改动对**下一个 sub-agent** 立即生效，不必重启。
   - "由主模型给探索任务生成 prompt"本就是现状：`task(agent, prompt)` 的 prompt 一直由主模型写，子代理看不到父对话——这是它省 token 的前提，不是缺陷。
2. **`✻ thought for Xs` 可展开**（点击折叠块，与工具输出同一 `push_with_detail` 机制），live 与 resume 重放共用 `bake_thinking`，重放不知道耗时故只报规模。**它确实是思考内容**，不是工具调用前的说明：Anthropic `thinking_delta`、DeepSeek `reasoning_content`、Codex `reasoning_summary_text`（这家给的是推理**摘要**）——三者都只映射到 `StreamEvent::ThinkingDelta`，正文走 `TextDelta`，两条流不会混。
3. **`no response headers within 20s` 的真凶是连接池，不是工具计时**。超时只裹 `connect_once`（发请求→收到响应头），工具执行不在里面。但两次模型请求之间隔着**几分钟的工具执行**，池里的连接一直闲着：NAT/代理悄悄掐掉后，池子照样把这条死连接发出来，请求写进去再也等不到回复——于是报错总在长工具调用之后出现，看起来"把工具时间算进去了"。修法是让死连接在被复用前就暴露：HTTP/2 idle ping + TCP keepalive + 45s 池空闲上限（`providers/http.rs`，三家共用一个 client）。同时 header 预算 20s → 90s：很多网关**不到首 token 不吐 header**，推理模型啃大 prompt 时几十秒是正常的，砍掉它并不能救活请求，只是杀死一个正在干活的请求、丢掉已经计费的 prompt 处理、然后重试进同一个等待。
4. **工具提速**：`read`/`write`/`edit` 的文件 IO 从阻塞 `std::fs` 换成 `tokio::fs`——此前 `join_all` 的"并行只读批次"其实是**串行**的（每个 future 在一次 poll 里同步跑完），还堵住 runtime 线程；freshness 锁改为短临界区（`MutexGuard` 跨 `await` 会让 future 非 Send，且会把整批写序列化在一个文件的磁盘延迟后面）。`edit` 失配回退路径 `find_ws_normalized` 曾对每个滑窗位置重算行 key，大文件上是**分配级 O(n·m)**，现在每行只算一次。`edit` 成功后的结果片段改用 plan 记录的匹配偏移定位——顺带修掉一个真 bug：旧代码回头 `find(new_string)`，当 new_string 在文件更早处也出现时，回显的是错误的行区间。
5. **`SlashCommand::hidden()` + `/dogfood`**：hidden 命令不进 `/help` 与补全，但照常 dispatch。`/dogfood` 开关一段 system prompt 附加块（`prompts/dogfood.md`），让模型在干活之余顺手报告**它自己刚撞上的**工具缺陷（错误信息害它多跑一轮、参数得靠猜、输出格式浪费 token），要求每条都能指认本会话里的具体调用，没有就不写。放 system prompt 而不是每轮 tail：切换时一次性重打缓存前缀（与 `/compact` 同量级），而 tail 方案要为同一段指令**永久按轮付费**并把它堆进历史。
   - 状态可见且持久：开启期间状态行常显 `· dogfood`（一个悄悄改变模型行为的模式，不能只在切换那一行出现过），开关值落 state.toml——**要人每个会话记得重开的模式，就是人会忘记开的模式**。写盘由前端做（`CommandEffect::PersistDogfood`，与 `/model` 的持久化同一分工）：core 的单元测试不该为了跑一条命令去写开发者的 home。
