...
- **自定义 agent（`.tcode/agents/*.md`）**：用户用文件定义 sub-agent 人设，与 skills 同构发现（`.tcode/agents` → `.claude/agents` → 两个 home 变体，first-wins）。frontmatter：`name`/`description`/`tools`（allowlist，逗号分隔）/`agents`（可 spawn 的 kind，即嵌套授权）/`model`|`profile`|`effort`（钉选，经 `config.agents.entry().or_insert()` 合并为默认值，手写 config 与 `/agents` 优先）/`readonly`/`max_steps`/`max_exchanges`（可追问轮数），正文 = system prompt。**统一表示**：builtin explore/plan/general 与 custom 都是 `AgentDef`，进同一 `AgentRegistry`（`crates/tcode-tools/src/agent_defs.rs`），task.rs 里三处按 kind 的 match（system/工具过滤/permission+batch）坍缩为查 def——custom 不是特例。**与 skills 覆盖语义刻意不同**：explore/plan/general 是保留字，文件不许覆盖（builtin 的"read-only 永不询问"权限语义绑定在 `read_only` 上，覆盖会静默放宽）。**嵌套**：def 声明 `agents` 才拿到受限子 `TaskTool`（`allowed` 限定 spawn 集，`depth` 上限 3 封死递归，不做图分析）。**多轮追问**：`max_exchanges > 0` 的 def 在 run 结束后把 Agent+Session 存进进程内 `live` map（cap 8、最旧逐出），工具输出带 resumable id 与余量；`resume` 参数对同一 session 纯 append 续聊（同 cache scope，命中前缀缓存，成本仅增量）。**headless**：`tcode --agent <name>` 用该 def 的 system/工具集/模型跑一次性 session（与 `--continue`/`--resume` 互斥），驱动脚本靠它做批量回放。示例 `examples/invest-replay/`（investor 委托 quant-dev 回测并可追问、driver 侧独立 auditor 审查未来函数、物理数据快照隔离 as-of）。
...
3. progress以及过程中context超过100%了都没自动压缩
4. Explore task你这个绿色画错位置了，你现在画在agent tree了，我是让你画在transcript, 那里的Explore还是灰色，或者我下次想让你知道就是主要对话的渲染区域，应该叫什么
5. edit的窗口不应该是allow for this session, allow for this project, 应该是allow all edits, 如果选了这个就自动切换到allow-edit的mode
