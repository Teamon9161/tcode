...
3. • functions.edit — 对重复的 descriptor, summary 片段做精确替换时，工具因匹配项不唯一而拒绝执行，随后需要额外读取上下文并重发编辑。错误信息已经说明原因，但如果返回每个候选的更短上下文和可直接选择的行号，能减少这类接口迁移中的一轮定位。
• functions.bash — 工作区测试输出被截断到 harness 文件，导致需要额外读取该文件才能定位两项失败测试。若截断响应能在末尾始终保留失败测试名、错误摘要和退出码，会避免额外读取。
4. 鼠标选中对应工具的记录展示的时候那个下划线太丑了，我觉得应该是有个常用的选中背景色，而且只要到有文字的部分，不要整行，然后展开的内容更不要有下划线了，很难看。
