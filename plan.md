...
3.  Tool friction — shell: 最终验证命令包含 git status --short 和 git diff --stat，但成功测试输出被自动折叠后，返回内容没有保留这两项状态结果，导致无法从该次输出确认工作区摘要。最小改进是在折叠成功输出时优先保留每条复合命令的最后非空输出块，或按子命令分段展示摘要
4. 比如agent tree调用Explore sub-agent那个Explore标签不需要上色，现在是绿色
5. 如果我在plan mode或者plan agent说参考xx(可能是网上git 项目，或者本地项目)，在做计划前应该先看过项目（网上的先clone下来看），本地的直接看
，看完了再出计划，而不是在计划里说phase 1是先clone下来看怎么做，模型应该直接看完，然后给出最终计划。  大概就是这个意思，你可以看下怎么写得精炼点，更新我们的prompt.
6. claude-code rules?
7. readonly sub-agent需不需要放开在scratchpad做一些探索的限制，只要不改项目就可以了？比如session的scratchpad下有个文件夹作为sub-agent的scratchpad
8. 用户在对话框输入note内容的时候，移动光标对导致光标右边的字往后面移动，光标自身不应该不占位置吗，现在感觉还占了半个空格的感觉
9. 对于tool调用展示，比如单行的Run(xxx)，现在只支持鼠标单击，希望可以支持鼠标长按拖动复制，现在拖动的时候还是和点击一个效果，不能复制。好像整个transript的复制都失效了
10.  Auto Mode paused after 3 consecutive classifier failures; permission mode is now default. Check the auto model in /agents or its connection, then re-enable Auto Mode when it is working., 但是ui显示的mode还是auto
