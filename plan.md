...
## 改进
2. edit — 我首次向 crates/tcode-core/src/auto_mode/mod.rs 插入模块导出时，目标 import 在生产代码和测试模块中各出现一次，工具只返回“old_string appears 2 times”，导致我额外补了一次带上下文的 edit。可以改进为在歧义错误中直接显示每个候选的少量上下文和行号，或支持 occurrence 参数，减少一次定位往返。
3. 用户prompt的显示字体过于大，而且过于高亮了太刺眼。
4. 100轮就停止或者报告，这个能不能默认先关闭，后面可能要改进一下，但是现在先默认不开。
5. compact完成后能不能有一个地方可以展开查看compact后的内容。compact之后要换行留个间隔。
6. 切换模式的时候能不能顺便记录到config中，下一次默认还是这个模式。
7.  functions.edit — 我在一次同文件批量编辑中误放入了“新旧字符串相同”的无操作替换；工具将它视为首项失败并跳过后续 7 个独立编辑，造成额外的读取和编辑回合。建议批量编辑在执行前单独报告并跳过 no-op 项，继续执行其他互不依赖的替换；或者将 no-op 作为非失败结果返回。
8. 有load-skill之类的tool吗，或者模型决定load_skill的时候用的是什么呢
9. 检查下Sub Agent的那个session_id还是缓存key是否同主agent分开，可以参考下auto模式的那个的实现，我怕影响主模型缓存，然后compact用的是同一个session id还是缓存key吗？这个应该用和主模型一样的？方便命中缓存对吧
10. explore task的展开结果也应该markdown渲染。是不是能增加一个好的交互方式，比如能够点击切换到这次explore的agent界面来看到prompt和工具调用结果之类的（这个切换对话记录显示的功能可以考虑做得通用点？方便之后可能支持多个agent并行的时候可以点击切换对话记录）
11. shell — 我一次性请求了多个大文件的完整 diff，返回被截断并保存到 scratch 输出文件，导致需要再用定向搜索和读取补齐信息。若工具能在输出中优先附带“按文件的改动摘要与新增文件列表”，或允许直接按文件限额输出 diff，会更适合审阅大量未提交改动。
