...
3. **已完成：Explore / Plan task 支持并行。** `task(agent="explore" | "plan")` 按输入声明为 `ParallelReadOnly`，同一轮多个只读子代理并发执行，结果仍按模型调用顺序写入 ledger 和重放。`general` 保持 isolated，绝不绕过审批或副作用边界。
4. **不做 Cargo 自动放行。** 即使 `cargo check --offline` 仍可能执行 build script 和 proc macro；离线只排除网络，不能成为 Auto Mode 的安全例外。分类器拒绝后可请求授权或选择真正无副作用的检查，但不应由 harness 猜测、改写或自动执行替代命令。
5. **已完成：session 私有 scratch 的运行与清理免审批。** scratch 现在是 `<project-data>/scratchpad/runs/<session-id>/`，环境注入、blob 输出、后台日志、TUI 粘贴和计划编辑均使用这一根目录。Auto Mode 对路径在当前 session scratch 内的文件操作，以及 `cwd` 位于该目录内的 shell 临时工作走 fast path；它不是 OS sandbox，scratch shell 命令不得借绝对路径触及外部。
6. **已完成：新目录指令只阻塞受影响的 mutation。** memory discovery 现在带有结构化作用根目录；同批 read 和无关写入继续运行，落入新指令 scope 的写入返回明确的未执行结果。含局部阻塞的批次退回逐调用路径，以保留审批、hook、checkpoint 和 ledger 顺序。
7. **已完成：grep/glob 跳过更多缓存树。** 共享 walker 现在额外排除 Zig 的 `zig-cache`、`zig-out`、`.zig-cache`，以及 Python、Node/前端的常见 cache/build 目录。原始 shell `find` 的输出不作意图猜测；应优先使用有分页、目录过滤和预算的 `glob`/`grep`，shell 大输出仍由 blob gate 落盘并保留 head/tail。
8. plan交互改进,我想给plan模式最后的No选项add note, 但是按tab变成又进入计划中了,还是对最后一段comment, 然后很重要的问题是plan预览窗口不支持鼠标滚轮上下翻,而且我在填写最后选项的note的时候,不支持上下翻动看前面的plan内容
