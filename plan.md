...
## 改进
1. Task比如Explore什么的，是不是应该支持使用别的模型进行探索,然后模型调用探索任务的时候,可以由模型给探索任务一个prompt.
2. thought for xx s,这块消息,是不是可以支持点击展开,支持查看模型思考了什么,还有这部分真的是thought吗,不会是工具调用前的简单说明吧.
3. API error (1/5): no response headers within 20s
这个消息感觉经常出现,是不是把工具调用的时间也算到里面去了呢,感觉不像正常的没连接上的报错.
4. 工具性能优化,尤其是Edit之类的.
