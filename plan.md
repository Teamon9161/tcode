...
3. **不做 Cargo 自动放行。** 即使 `cargo check --offline` 仍可能执行 build script 和 proc macro；离线只排除网络，不能成为 Auto Mode 的安全例外。分类器拒绝后可请求授权或选择真正无副作用的检查，但不应由 harness 猜测、改写或自动执行替代命令。
4. • functions.edit — 我用一个只包含 let mut d = plan_dialog(...) 的替换目标恢复测试函数头时，工具返回该文本出现 3 次，导致额外一次精确定位与编辑。可增加按匹配序号或行号限定替换目标的参数，避免为常见重复文本额外读取上下文。
• functions.edit — cargo fmt 把断言格式化为多行后，我按格式化前的一行文本修改测试预期，工具只返回“old_string not found”，需要额外读取文件以确认新格式。可在失败时返回最相近的匹配片段或该字符串附近的格式化差异，减少一次读取。
 functions.edit — 我按格式化前的多行测试文本做替换时，工具返回 old_string not found，虽然给出了最接近片段，仍需要额外读取当前范围才能安全修改。为失败结果提供可直接复用的完整候选替换块，或支持“近似匹配后确认”，可以省掉这次重新读取。
5. 在edit或者类似工具按tab输入note的时候，如果我用中文输入法在打字，光标不会随着打的英文字母而前进。
