...
## 改进
1. 完成的plan是不是可以用贯穿文字的删除线，然后颜色上也可以有所变化
2.  API error (1/3): stream stalled: no data for 30s
↻ API error (2/3): network error: error sending request for url (https://chatgpt.com/backend-api/codex/responses)
之前模型已经输出很多内容了，然后突然API error把之前输出的内容都清屏了，这感觉是不是不太合理，输出一半的内容也先留着会不会更好？感觉也是有信息的，如果我觉得中断的回答不好，我手动双esc rewind就好了，这也就相当于这块输出去掉了对吧？直接删了的话内容感觉就丢失了。
3. note to model也是user的prompt，这块在渲染ui的时候也要加user prompt相同的背景，避免被忽略。
4. 输入法打中文的时候，打到一半还没选择输入，但是这种情况下光标不会跟着移动。也不会显示打到一半的英文输入。
