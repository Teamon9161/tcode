...
6. sub-agent应该有个参数支持能把AskUser提到主对话来，默认是不允许，但是这对于plan agent很重要，规划前有些问题必须得找用户问清楚。
7. plan mode支持No的时候不自动跳转到加note功能， 如果我选了No, 但是没加note那就是先暂停对话。和我权限选择的时候选no, 然后没有note的时候一样，直接先中断对话。（或者这个时候直接按Esc也应该中断对话）
8. claude-code rules?
