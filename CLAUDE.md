...
- `[agents.explore]` / `[agents.general]`：给 sub-agent 钉模型（`profile`/`model`/`effort` 可选，未写的继承父选择）。钉住的 kind 不跟随 `/model`；未配置的 kind 共享父 `ModelCell`。运行时经 `AgentModels`（可换手柄）共享给 `task` 工具与 `/agents` 选择器，改动即刻对下一个 sub-agent 生效。
