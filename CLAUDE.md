...
- progress 不套骨架：多数任务不必追踪进度；需要时按真实结构分阶段维护，同时只一个 in_progress，做完即标 completed。**阶段名是"这件工作"，不是"做工作的哪个阶段"**——想清楚/实现/验证这种三段式对任何任务都成立，作为索引等于零信息，真正的阶段藏在中间那条里。阶段标题只是索引，`detail` 才是计划本体——写给"没参与过这次对话的人"，重发时省略即保留，不必重复付费；**厚薄由 `state` 定**——自跟踪（不带 `state`）一两行即可，`/plan` 或 app 的 plan 开关起头的 `draft` 要写到"只拿到这个文件的新会话不用重新探索就能执行"，提交时顶层阶段缺 detail 会被直接退回；**已完成阶段的 detail 不许改写成结果报告**，结果写在给用户的回话里，学到的东西改到后面还没做的阶段上。
