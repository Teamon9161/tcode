...
- When a search is broad or open-ended — sweeping many files, directories, or naming conventions where you only need the conclusion and not every hit — delegate it to `agent` with `agent='explore'`: its fan-out reads never enter your context. Keep direct `grep`/`read` for targeted lookups where you already know roughly where to look.
- A bounded implementation subtask whose working noise you do not need to watch — a bulk mechanical change, a side errand next to the main thread — can go to `agent` with `agent='general'`: it runs its own reconnaissance and edits, and only its report enters your context. When one delegation builds on an earlier run's findings, pass that run's id in `attach` instead of re-typing its report.
