...
- **"哪些文件可以被显示"只有一个定义，`viewable_within` 是它的另一个返回形态**（落在哪个 root 下 + 相对路径），`is_viewable_path` 就是它的 `.is_some()`。桌面端拿它当本机 origin 的 mount，所以两个返回值必须一起给：调用方**算不出**相对路径那一半——containment 是逐 component 且 Windows 上大小写不敏感的，`strip_prefix` 恰好对这个函数接受的每一对都返回 `None`。新增可显示的位置改这一个函数，别在调用方那边补 if。
...
- **权限分两个正交旋钮，别合并**：模式与 allow/ask/deny **继承自父会话**（经 `ToolCtx::delegated_permissions`，`forward_delegates` 按调用装卸），因为委派出去的活仍是本会话的活——子 session 曾自建 `PermissionMode::Auto` + 空规则，等于用户的 deny 规则对子 agent 静默失效；而 `readonly` 是 def 自己的**能力天花板**，在 `sub_tools` 里就摘掉 mutating 工具，比模式更强（模式能被用户点 yes 抬高，天花板连请求都不存在），所以 explore 在父是 unsafe 时仍动不了项目。审批桥对**所有**委派运行安装，`questionPolicy` 只管 `ask_user` 工具——把两者绑一起会让"继承一个会询问的模式"静默变成"会拒绝"（`NeverAsk`）。
...
- **每次委派可临时覆盖 model/effort，但解析器由 composition root 注入**（`agent` 工具的可选 `model`/`effort` 入参 → `resolve_override` → `ModelResolver`）：工具本身只握 `ModelCell`（父模型）与 `pinned`（已解析的 pin），够不到 profile 目录，所以按名字解析任意模型的能力靠 `with_model_resolver` 从 main.rs 注入（闭包捕获 `Config` 快照 + 父 `Selection`，走 `Config::resolve_model_override` + `build_active`）。这符合"状态与策略按最窄使用范围归属"：解析逻辑归 composition root，工具只消费 `ActiveModel`。三条不可省：① 覆盖只作用于**新起的 run**，`resume`/`restore` 一律传 `None`——停靠的 session 绑死在它当初的模型与 cache scope 上，中途换模型既毁缓存又语义不清；② 未注入 resolver 却收到 `model`/`effort` 要**自愈报错**（提示去 `/agents`/`/model` 钉），不能静默回落到 pin——模型点名了某模型就必须知道没被满足，而不是以为在它上面跑过了；③ 未编入目录的模型名**原样透传**给 provider（与 config pin、`/model` 一致，endpoint 可能认识我们没编目的模型）。快照语义：resolver 捕获的是启动时的 `Config`，会话中途 `/provider` 新增的 profile 不反映——可接受（profile 一次配好），别为此把整个 Config 做成共享可变。
