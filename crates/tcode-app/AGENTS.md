...
9h. **浏览器窗格是原生 WebContentsView，一个 tab 一个 view，整体是窗口级单例**（`electron/browser.js` + `ui/src/webHost.ts` + `ui/src/web.ts` + `ui/src/WebPane.tsx`，`Pane` 的第三个变体 `{kind:"web"}`）。
    ...
    **截图消费能力按 provider 的真实传输能力动态路由，而不是按工具是否注册静态裁掉。** `supports_vision` 只说明模型能在 user message 里看图；`supports_tool_result_images` 另说明 API 能不能把图片放进 tool result，二者不可混为一个能力。两者都成立（当前是 Anthropic 与 Codex Responses）时 `screenshot` 直接回 image block；OpenAI Chat Completions 的 `tool` role 仍是纯文本，因此改用 live `[agents.vision]` 的隔离请求并让主会话只收到 `<web-page-content>` 里的文字观察。两条路线都不可用时在 capture 前明确拒绝并指向 `snapshot` 与 `/agents → vision`。`snapshot` 是 AX 文本/结构/ref，和 vision 能力无关，始终保留。role pin 必须共享 `tcode-frontend::boot` 解析出的同一个 `AgentModels` handle，不能在 app 里另读一份启动配置，否则 `/agents` 改完不会生效。

    **截图的可选 viewport 是一次性观测，不是改变窗格布局的 `resize` action。** `width`/`height` 必须成对且限制在 320–2560 × 240–1440 CSS px；Rust 与 Electron 壳各校验一次。`rendered` 即使面对当前可见 tab 也先把目标放到 app renderer 下方，再按请求尺寸产帧与截图，finally 恢复真实 pane rect 和 native z-order，所以 responsive 检查不会闪动或永久改掉用户正在看的页面。
