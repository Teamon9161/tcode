 ...
 * `"shown_file"`, `"sessions"` and the rest — so this does **not** wrap calls
 * in the envelope the real seam adds through the preload. That envelope exists
 * to reach a Rust registry over Electron's pipe, and the preview has no backend
 * to reach: putting it in here would mean every fixture had to unwrap it before
 * recognizing its own command.
 *
 ...
 * window here to drag. Naming the preview rather than borrowing the app's name
 * is what keeps that true — the real app answers `"electron"`, which would
 * hand the preview a drag region that swallows the clicks of the menus sitting
 * in the bar, exactly what the preview exists to look at.
 */
