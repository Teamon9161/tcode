...
- **每条 harness note 的第一行必须是一句能独立成立的话**（`background.rs` 里那几个 note 构造函数）：正文其余部分是写给模型的——要重跑的命令、要带 offset 读的日志路径、关于输出重定向的提示。它上屏是因为"手表响了"这件事人也该知道，而**这句话就是第一行**；桌面端据此折叠（`tcode-app` 规则 19c）。新写一条把要点埋到第三行的 note，会静默毁掉每一处折叠渲染——那不是格式约定，是这些 note 能同时寄给两个读者的**前提**。
...
- **一份计划是包含阶段表的文档，不是阶段表**。三层，各有各的下发规则：front matter 的 `description`（一行，只有 inventory 与 `/plan list` 用它，它是"不必打开就能判断相关性"这句话的全部依据）；`background`（自由 markdown，写在阶段之前，装一切不属于任何单个阶段的东西——决策与理由、勘查确立的事实、核心数据结构、贯穿的约束、被否掉的方案）；`phases` 与各自的 `detail`。**不要把 `background` 做成固定小节表**（背景/风险/验证）：那对任何任务都成立，等于零信息，正是本仓库拒绝阶段三段式的同一条理由。
  这不是加个字段的便利。没有它，横切的正文只有三条去处：塞进第一个阶段的 detail 当伪前言、在每个阶段里重抄一遍、或者**模型干脆绕过这个工具用 `write` 另写一个 md**——第三条是被观察到的行为，而那一刻这个工具就不再描述这项工作了。
- **第一个带方框的标题之前的一切都是 `background`**（`parse_body`）。它曾经被直接丢弃，于是用户或评审者写在清单上方的前言会被下一次阶段翻转静默删掉——正是 `reconcile` 那句"用户版本权威"要挡的事。渲染顺序永远是正文在前、阶段在后：清单下方追加的小节会被读成最后一个阶段的 detail，反过来排就让一次 `$EDITOR` 往返不再等于它看上去的意思。
...
- **`background` 也在摘要里下发，而且没有更窄的时机可选**：它按定义不属于任何阶段，所以"某个阶段刚开始"对它不成立，唯一的触发点就是"有个会话正在接手这份文件"。代价诚实地兜在 `SUMMARY_BACKGROUND_BUDGET`：超预算就退化成小节标题加一句"无参调用 `progress` 看全文"，因为摘要每次 compact 都要重付一遍。**退化时 `background_seen` 为假**——只被指过路的正文仍然是没看过的正文，改不得。
- **写文件的工具也得能读文件**：没有 `phases`、`state`、`background`、`description` 的调用 = 看全文（`full_view`，所有阶段所有 detail）。模型对这份文件没有别的读法——路径要猜，`read` 又与"工具是唯一写入者"冲突。别把它做成新参数：无可施加的内容时，诚实的解释就是"给我看看"。**重复看走 read 的同一条契约**：body hash 没变就回 stub 指向上文，变了才重发（`viewed`）。它不需要 `read` 那个 `force` 逃生口，因为"上文那份还在不在"由 `summary()` 一处判定——见下条。
...
- **`background` 与 `description` 走 `detail` 同一条"省略即保留 / 没看过不许改"的规则**，理由一字不差：`phases` 每次全量重发，任何必须随行的正文都会被重复计费，模型于是理性地把它压成空。`background_seen` 是独立的 bool 而不是 `seen` 里的哨兵 key——阶段标题是用户可写的文本，任何哨兵都可能正好是某个阶段的名字。
- **正文的厚薄由 `state` 决定，不由模型对任务大小的判断决定**：没有 `state` 的自跟踪文件写给"还握着全部上下文的这次对话"，一两行足够、`background` 可空；`draft` 的读者按定义是别人——审批的人，以及可能只拿到这个文件的新会话（`plan::execution_instruction`）——三层必须都写满。因此 `review_copy` 是唯一一处对缺席报错的地方（`submission_gaps`：`description`、`background`、以及顶层阶段的 `detail`；子阶段是在父阶段正文里读的），在人被打扰之前退回，**三种缺口一次报全**——一个字段一次往返是在让模型花钱问出我们本来就握着的清单。**这道闸不许退化成 tool description 里的一句话**：写正文的成本是模型此刻的输出 token，收益归一个它永远见不到的读者，纯 prompt 纪律必然被省掉——与上一条是同一种经济学的两半。
