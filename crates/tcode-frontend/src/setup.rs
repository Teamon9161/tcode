...
//!
//! Three renderers drive it: the standalone first-run wizard (`wizard.rs`,
//! which must work before an `App` exists because there is no usable model
//! yet), the in-session `/provider` overlay, and the desktop app's own form.
//! Keeping the decisions here means they agree on what a keystroke does and on
//! what lands in `config.toml`; they differ only in how a `View` is painted.
...
/// The keystrokes setup distinguishes, named for what they mean here rather
/// than in any one input stack. Each frontend maps its own events onto this —
/// the TUI translates crossterm `KeyEvent`s, the desktop app browser events —
/// which is what keeps this crate free of a terminal dependency.
...
pub enum Key {
    ...
    Tab,
    Backspace,
    /// A printable character. Modified chords (Ctrl+X) are not this — they map
    /// to `Cancel` or `Other`, so they never land in a text field.
    Char(char),
    /// Esc, Ctrl+C, or whatever else the frontend calls "back out".
    Cancel,
    /// Anything setup ignores. Frontends map the rest of the keyboard here
...
impl Setup {
    ...
    pub fn on_key(&mut self, key: Key) -> Progress {
        if key == Key::Cancel {
            return self.cancel();
        }
        match &mut self.step {
            ...
            Step::Key { .. } if matches!(key, Key::Enter | Key::Tab) => self.confirm_key(),
            Step::CustomName { .. }
            | Step::CustomBaseUrl { .. }
            | Step::CustomModels { .. }
            | Step::Key { .. }
            ...
            Step::CustomProtocol { cursor, .. } => {
                ...
                match key {
                    ...
                    Key::Enter => return self.confirm_protocol(),
                    _ => {}
                }
                Progress::Stay
            }
            Step::Model { options, cursor } => {
                ...
                match key {
                    Key::Up => *cursor = cursor.saturating_sub(1),
                    Key::Down => *cursor = (*cursor + 1).min(last),
                    Key::Enter => return self.confirm_model(),
                    _ => {}
                }
                Progress::Stay
            }
    ...
    fn providers_key(&mut self, key: Key) -> Progress {
        ...
        match key {
            ...
            Key::Enter => return self.confirm_providers(),
            _ => {}
        }
        Progress::Stay
    }

    /// [`Key::Cancel`]. Inside a sub-step it backs out to the provider list —
...
/// Progress of a `/login` run, delivered to a frontend from the injected flow.
/// The concrete OAuth work lives in the composition root (like `SwitchFn`), so
/// no frontend needs the provider implementations.
...
/// `Arc` + boxed future so a frontend can spawn it on the runtime.
...
mod tests {
    ...
    fn press(setup: &mut Setup, keys: &[Key]) -> Option<Box<Config>> {
        let mut result = None;
        for &key in keys {
            if let Progress::Done(outcome) = setup.on_key(key) {
                result = outcome;
            }
        }
        result
    ...
    fn a_builtin_profile_is_saved_as_a_key_only_patch() {
        ...
        setup.on_key(Key::Tab);
        type_str(&mut setup, "sk-typed");
        // key → back to the list, confirm the list, confirm the model.
        let saved =
            press(&mut setup, &[Key::Enter, Key::Enter, Key::Enter]).expect("setup completes");
    ...
    fn a_custom_endpoint_is_written_whole() {
        ...
        setup.on_key(Key::Enter); // name → protocol
        setup.on_key(Key::Enter); // openai
        type_str(&mut setup, "https://api.groq.com/openai/v1");
        setup.on_key(Key::Enter); // base url → models
        type_str(&mut setup, "llama-4, mixtral");
        setup.on_key(Key::Enter); // models → key
        type_str(&mut setup, "sk-groq");
        setup.on_key(Key::Enter); // key → back to the list

        let saved = press(&mut setup, &[Key::Enter, Key::Enter]).expect("setup completes");
    ...
    fn escaping_a_field_keeps_the_selection() {
        let mut setup = Setup::new(Config::default(), None);
        go_to(&mut setup, "deepseek");
        setup.on_key(Key::Char(' '));
        setup.on_key(Key::Char('c'));
        type_str(&mut setup, "half-typed");
        setup.on_key(Key::Cancel);
        ...
        assert!(matches!(setup.on_key(Key::Cancel), Progress::Done(None)));
    }
    ...
    fn reconfiguring_one_profile_leaves_the_model_choice_alone() {
        ...
        let saved = press(&mut setup, &[Key::Enter]).expect("setup completes");
        let config = *saved;
    ...
    fn confirming_an_empty_selection_cancels() {
        ...
        assert!(matches!(setup.on_key(Key::Enter), Progress::Done(None)));
    }
}
